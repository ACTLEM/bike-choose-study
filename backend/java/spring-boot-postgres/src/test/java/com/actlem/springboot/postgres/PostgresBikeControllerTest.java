...
class PostgresBikeControllerTest extends PropertyTest {

    @Mock
    private PostgresBikeService bikeService;
    ...
    void findByReturnsBikesFromService(
            @RandomObject BikePage<PostgresBike> bikePage,
        ...
        ResponseEntity<BikePage<PostgresBike>> response = cut.findBy(types, genders, bikeBrands, frameMaterials, forkMaterials,
    ...
    void searchBikesReturnsFromService(
        ...
        SearchResult<PostgresBike> searchResult = new SearchResult<>(bikePage, facets);
        when(bikeService.search(eq(pageable), filterListCaptor.capture())).thenReturn(searchResult);

        ResponseEntity<SearchResult<PostgresBike>> response = cut.search(types, genders, bikeBrands, frameMaterials, forkMaterials,
                brakes, cableRoutings, chainsets, groupsetBrands, wheelSizes, colors, pageable);
