...
class ElasticsearchBikeControllerTest extends PropertyTest {
    ...
    void saveCreateBikeViaService(@RandomObject ElasticsearchBike bike) {
        when(bikeService.save(bike)).thenReturn(bike);

        ResponseEntity<ElasticsearchBike> response = cut.save(bike);
    ...
    void findByReturnsBikesFromService(
        ...
        ResponseEntity<BikePage<ElasticsearchBike>> response = cut.findBy(types, genders, bikeBrands, frameMaterials, forkMaterials,
    ...
    void searchBikesReturnsFromService(
        ...
        SearchResult<ElasticsearchBike> searchResult = new SearchResult<>(bikePage, facets);
        when(bikeService.search(eq(pageable), filterListCaptor.capture())).thenReturn(searchResult);

        ResponseEntity<SearchResult<ElasticsearchBike>> response = cut.search(types, genders, bikeBrands, frameMaterials, forkMaterials,
                brakes, cableRoutings, chainsets, groupsetBrands, wheelSizes, colors, pageable);
