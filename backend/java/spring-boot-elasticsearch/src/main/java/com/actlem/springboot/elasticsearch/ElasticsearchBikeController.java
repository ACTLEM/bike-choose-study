...
public class ElasticsearchBikeController {
    ...
    private final ElasticsearchBikeService bikeService;

    /**
     * Endpoint to create a {@link ElasticsearchBike}
     */
    @PostMapping
    public ResponseEntity<ElasticsearchBike> save(@RequestBody ElasticsearchBike bike) {
        return new ResponseEntity<>(bikeService.save(bike), HttpStatus.CREATED);
    }

    /**
     * Endpoint to find {@link ElasticsearchBike} stored in the repository with a pagination (size=20 by default) and optional filters
     */
    @GetMapping
    public ResponseEntity<BikePage<ElasticsearchBike>> findBy(
    ...
    /**
     * Endpoint to get possible values to filters {@link ElasticsearchBike} stored in the repository, a.k.a facets, and according to filters.
     */
    @GetMapping("facets")
    ...
    /**
     * Endpoint to search {@link ElasticsearchBike} and {@link Facet} in the repository, and according to filters.
     */
    @GetMapping("search")
    public ResponseEntity<SearchResult<ElasticsearchBike>> search(
            @RequestParam(required = false, defaultValue = "") List<Type> types,
            @RequestParam(required = false, defaultValue = "") List<Gender> genders,
            @RequestParam(required = false, defaultValue = "") List<BikeBrand> brands,
            @RequestParam(required = false, defaultValue = "") List<Material> frames,
