...
class SolrBikeControllerTest extends PropertyTest {
    ...
    void findByReturnsBikesFromService(
        ...
        ResponseEntity<BikePage<SolrBike>> response = cut.findBy(types, genders, bikeBrands, frameMaterials, forkMaterials,
    ...
    void searchBikesReturnsFromService(
        ...
        SearchResult<SolrBike> searchResult = new SearchResult<>(bikePage, facets);
        when(bikeService.search(eq(pageable), filterListCaptor.capture())).thenReturn(searchResult);

        ResponseEntity<SearchResult<SolrBike>> response = cut.search(types, genders, bikeBrands, frameMaterials, forkMaterials,
                brakes, cableRoutings, chainsets, groupsetBrands, wheelSizes, colors, pageable);
